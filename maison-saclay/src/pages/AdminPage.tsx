...
import { LogIn, RefreshCw, CheckCircle, XCircle, Clock, Mail, Calendar, Users, Euro, ChefHat, ShoppingBag, Truck, Wifi, Monitor } from "lucide-react";
...
import {
  ...
  type WifiRadiusData,
  type WifiRadiusSession,
  type WifiActivityData,
  type WifiActivityLog,
} from "@/services/api";
...
// ── Activité réseau (DNS logs ou démo) ───────────────────────
function ActivitySection({ data }: { data: WifiActivityData | null }) {
  const logs  = data?.logs  ?? [];
  const stats = data?.stats ?? { total: 0, real: 0, demo: 0, has_real_data: false, last_activity: null };
  ...
  return (
    <div className="mt-8">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-light text-charcoal/60 tracking-luxury uppercase flex items-center gap-2">
          <Monitor size={13} />
          Activité réseau
        </h3>
        {stats.has_real_data ? (
          <span className="text-2xs px-2 py-0.5 bg-emerald-50 border border-emerald-200 text-emerald-700 font-light">
            Source réelle
          </span>
        ) : (
          <span className="text-2xs px-2 py-0.5 bg-amber-50 border border-amber-200 text-amber-700 font-light">
      ...
      {/* Note honnête */}
      {!stats.has_real_data && (
        <div className="mb-3 bg-amber-50 border border-amber-200 px-4 py-3 flex items-start gap-3">
          <Monitor size={14} className="text-amber-500 mt-0.5 flex-shrink-0" />
          <p className="text-xs font-light text-amber-800 leading-relaxed">
            <strong className="font-medium">Données de démonstration.</strong> Les entrées ci-dessous
            illustrent ce que cette section afficherait avec un DNS local connecté. Dans une infrastructure
            réelle, elles seraient alimentées par les logs DNS (Pi-hole, dnsmasq) ou le pare-feu réseau.
            Le contenu HTTPS reste chiffré et non lisible.
          </p>
        </div>
      )}
      ...
      <div className="bg-white border border-border overflow-hidden">
        {/* En-tête tableau */}
        <div className="px-4 py-2 bg-charcoal/3 border-b border-border grid grid-cols-5 gap-3">
          {["Chambre", "Adresse MAC", "Domaine consulté", "Protocole", "Heure"].map((h) => (
            <p key={h} className="text-2xs tracking-luxury uppercase text-charcoal/40">{h}</p>
          ))}
        </div>

        {logs.length === 0 ? (
          <div className="px-4 py-10 text-center">
            <Monitor size={24} className="mx-auto text-charcoal/15 mb-3" />
            <p className="text-xs font-light text-charcoal/40">Aucune activité réseau collectée</p>
            <p className="text-2xs text-charcoal/30 font-light mt-1">
              Les logs DNS ou pare-feu ne sont pas encore connectés
            </p>
          </div>
        ) : (
          logs.map((log: WifiActivityLog) => (
            <div
              key={log.id}
              className="px-4 py-2.5 grid grid-cols-5 gap-3 border-b border-border/50 hover:bg-charcoal/2 transition-colors"
            >
              <p className="text-xs font-light text-charcoal">Ch. {log.room_number}</p>
              <p className="text-xs font-mono text-charcoal/50 truncate">{log.mac_address ?? "—"}</p>
              <div className="flex items-center gap-1.5">
                <p className="text-xs font-mono text-charcoal/80 truncate">{log.domain ?? log.destination_ip ?? "—"}</p>
                {log.is_demo ? (
                  <span className="flex-shrink-0 text-2xs px-1 bg-amber-50 border border-amber-200 text-amber-600">démo</span>
                ) : (
                  <span className="flex-shrink-0 text-2xs px-1 bg-emerald-50 border border-emerald-200 text-emerald-600">réel</span>
                )}
              </div>
              <p className="text-2xs font-light text-charcoal/50">{log.protocol}</p>
              <p className="text-xs font-light text-charcoal/50">{fmtTime(log.visited_at)}</p>
            </div>
          ))
        )}
      </div>
      ...
      {/* Note architecture */}
      <div className="mt-3 bg-blue-50 border border-blue-200 px-4 py-3 flex items-start gap-3">
        <Monitor size={14} className="text-blue-500 mt-0.5 flex-shrink-0" />
        <div className="text-xs font-light text-blue-700 leading-relaxed space-y-1">
          <p>
            <strong className="font-medium">Supervision non intrusive.</strong> Seuls les domaines DNS sont
            visibles, pas le contenu des pages (chiffré en HTTPS). Les mots de passe et données privées
            ne sont jamais collectés.
          </p>
          <p className="text-blue-600/70">
            Pour des données réelles : DNS local (Pi-hole / dnsmasq) → forcer via DHCP → parser les logs
            → table <code className="font-mono bg-blue-100 px-0.5">wifi_radius_activity_logs</code>.
          </p>
        </div>
      </div>
    </div>
...
// ── Onglet WiFi invité — données RADIUS réelles ──────────────
...
function WifiTab({ data, activityData }: { data: WifiRadiusData | null; activityData: WifiActivityData | null }) {
  if (!data) {
    return (
      <div className="text-center py-20">
        <p className="font-serif text-2xl font-light text-charcoal/30">Chargement…</p>
      </div>
    );
  }
  ...
  const statCards = [
    { label: "Appareils",  value: n(stats.devices),  color: "text-charcoal" },
    { label: "Chambres",   value: n(stats.rooms),    color: "text-gold" },
    { label: "Acceptés",   value: n(stats.accepted), color: "text-emerald-600" },
    { label: "Refusés",    value: n(stats.rejected), color: "text-red-500" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((s) => (
          <div key={s.label} className="bg-white border border-border p-4">
            <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">{s.label}</p>
      ...
      <div className="bg-emerald-50 border border-emerald-200 px-4 py-3 flex items-start gap-3">
        <Wifi size={14} className="text-emerald-600 mt-0.5 flex-shrink-0" />
        <p className="text-xs font-light text-emerald-800 leading-relaxed">
          <strong className="font-medium">Données réelles.</strong> Le client saisit son numéro de chambre
          dans le portail captif intégré de la borne TP-Link. La borne interroge FreeRADIUS, qui valide
          la chambre et autorise l'accès Internet. Les connexions ci-dessous proviennent des logs RADIUS
          réels : chambre, adresse MAC, statut et heure.
        </p>
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-16 bg-white border border-border">
          <Wifi size={32} className="mx-auto text-charcoal/20 mb-3" />
          <p className="font-serif text-xl font-light text-charcoal/30">Aucune connexion WiFi</p>
          <p className="text-sm text-charcoal/30 font-light mt-1">
            Les authentifications par chambre apparaîtront ici après sync RADIUS
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {sessions.map((s: WifiRadiusSession) => {
            const accepted = s.auth_status === "Access-Accept";
            return (
              <motion.div
                key={s.id}
                initial={{ opacity: 0, y: 6 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-white border border-border p-4 lg:p-5 hover:border-charcoal/20 transition-colors duration-200"
              >
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-5 flex-1">
                    <div>
                      <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">Chambre</p>
                      <p className="text-sm font-light text-charcoal">Ch. {s.room_number}</p>
                    </div>
                    <div>
                      <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">Adresse MAC</p>
                      <p className="text-xs font-mono text-charcoal/70">{s.mac_address ?? "Non disponible"}</p>
                    </div>
                    <div>
                      <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">Borne (NAS)</p>
                      <p className="text-xs font-mono text-charcoal/40">{s.nas_ip ?? "—"}</p>
                    </div>
                    <div>
                      <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">Connexions</p>
                      <p className="text-xs font-light text-charcoal/70">
                        {s.auth_count}× · vu {fmtDateTime(s.last_seen_at)}
                      </p>
                    </div>
                    <div>
                      <p className="text-2xs tracking-luxury uppercase text-charcoal/40 mb-1">Première auth.</p>
                      <p className="text-xs font-light text-charcoal/70">{fmtDateTime(s.connected_at)}</p>
                    </div>
                  </div>
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-2xs font-light border tracking-wide flex-shrink-0 ${
                    accepted
                      ? "text-emerald-700 border-emerald-200 bg-emerald-50"
                      : "text-red-600 border-red-200 bg-red-50"
                  }`}>
                    {accepted ? <CheckCircle size={10} /> : <XCircle size={10} />}
                    {accepted ? "Accès autorisé" : "Refusé"}
                  </span>
                </div>
              </motion.div>
            );
...
export function AdminPage() {
  ...
  const [tab, setTab] = useState<"bookings" | "contacts" | "orders" | "inventory" | "stats" | "wifi">("bookings");
  const [bookings,  setBookings]  = useState<AdminBooking[]>([]);
  const [contacts,  setContacts]  = useState<AdminContact[]>([]);
  const [orders,    setOrders]    = useState<AdminOrder[]>([]);
  const [inventory, setInventory] = useState<InventoryData | null>(null);
  const [wifiRadiusData, setWifiRadiusData] = useState<WifiRadiusData | null>(null);
  const [activityData,   setActivityData]   = useState<WifiActivityData | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [nextBookings, nextContacts, nextOrders, nextInventory, nextRadius, nextActivity] = await Promise.all([
        fetchAdminBookings(),
        fetchAdminContacts(),
        fetchAdminOrders(),
        fetchInventory(),
      ...
      <div className="max-w-8xl mx-auto px-6 lg:px-12 py-10">
        ...
        <div className="flex gap-1 mb-8 border-b border-border">
          {([
            ...
            ["wifi",      `WiFi (${wifiRadiusData?.stats?.devices ?? 0} appareils)`],
        ...
        <AnimatePresence mode="wait">
                        ...
                        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-3 lg:gap-6">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-charcoal/40 font-light flex items-center gap-1">
                              ...
                              {booking.room_number && (
                                <span className="ml-1">· Ch. <strong className="text-charcoal/60">{booking.room_number}</strong></span>
                              )}
                            </p>
                            {booking.phone_extension && (
                              <p className="text-xs text-gold font-light mt-0.5">
                                ☎ Poste {booking.phone_extension} · Réception 1099
                              </p>
                            )}
                          </div>
                          <div>
          ...
          {tab === "wifi" && (
            <motion.div key="wifi" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <WifiTab data={wifiRadiusData} activityData={activityData} />
            </motion.div>
          )}
