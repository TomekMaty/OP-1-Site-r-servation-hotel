...
import { fetchMyBookings, fetchMyOrders, updateMyEmail, type MyBooking, type MyOrder } from "@/services/api";
import { cn, formatPrice } from "@/lib/utils";

type Tab = "bookings" | "orders" | "profile";
          ...
          <div className="max-w-lg">
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xs tracking-luxury uppercase text-charcoal/60">
                  Adresse e-mail
                </h2>
              </div>
              ...
              {!emailEdit ? (
                <div className="flex items-center justify-between">
                  <p className="font-light text-charcoal text-sm">{user.email}</p>
                  <button
                    onClick={() => {
                    ...
                    className="flex items-center gap-1.5 text-2xs tracking-wide text-charcoal/40 hover:text-charcoal transition-colors"
                  >
                    <Pencil size={12} />
                    Modifier
                  </button>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="flex-1 border border-border px-4 py-2.5 text-sm font-light text-charcoal focus:outline-none focus:border-charcoal transition-colors"
                    />
                    <button
                  ...
                  {emailMsg && (
                    <p className={cn("text-xs font-light", emailMsg.ok ? "text-emerald-600" : "text-red-500")}>
                      {emailMsg.text}
                    </p>
                  )}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (!emailInput.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)) {
                        setEmailMsg({ ok: false, text: "Adresse e-mail invalide." });
                        return;
                      }
                      ...
                      try {
                        ...
                        setEmailMsg({ ok: true, text: "E-mail mis à jour." });
                        setEmailEdit(false);
                      } catch (err: unknown) {
                    ...
                    className="w-full py-2.5 bg-charcoal text-ivory text-2xs tracking-luxury uppercase hover:bg-charcoal/80 transition-colors disabled:opacity-50"
                  >
              ...
              <p className="mt-6 pt-6 border-t border-border text-xs font-light text-charcoal/40 leading-relaxed">
                Votre adresse e-mail est utilisée pour les confirmations de réservation,
                les commandes room service et les messages vocaux.
              </p>
            </motion.div>
          </div>
        )}
