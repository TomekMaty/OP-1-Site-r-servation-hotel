...
export function BookingWidget({ room }: { room: Room }) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Redirection vers la page de confirmation premium
      navigate("/confirmation", {
        state: {
          id:              result.id,
          room_number:     result.room_number ?? "—",
          phone_extension: result.phone_extension ?? null,
          room_name:       room.name,
          room_category:   room.category,
          room_image:      Array.isArray(room.images) ? room.images[0] : "",
          check_in:        data.checkIn,
          check_out:       data.checkOut,
          nights,
          total:           result.total_price ?? nights * room.price,
        }
      });
    } catch (error: unknown) {
