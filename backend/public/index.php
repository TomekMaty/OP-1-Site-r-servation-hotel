...
use App\Controllers\VoicemailController;
...
// Portail WiFi (admin)
$router->get('/wifi-sessions', fn() => (new WifiController())->index());
$router->get('/wifi-radius-sessions', fn() => (new WifiController())->radiusSessions());
$router->get('/wifi-activity', fn() => (new WifiController())->activityLogs());
...
$router->put('/me/email', fn() => (new MeController())->updateEmail());

// Test SMTP (admin uniquement) : POST /api/admin/mail-test  body: {"to":"email@..."}
$router->post('/admin/mail-test', fn() => (new MeController())->mailTest());

// Notification voicemail Asterisk (appelé par voicemail_notify.sh)
$router->post('/voicemail-notify', fn() => (new VoicemailController())->notify());
