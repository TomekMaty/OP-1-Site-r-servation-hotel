...
import { fetchMenu, createOrder, type MenuItem, type MenuByCategory } from "@/services/api";
...
const CATEGORY_HOURS: Record<string, string> = {
  brunch: "Servi de 7h à 11h",
  plat: "Disponible de 11h à 22h",
  boisson: "Disponible toute la journée",
  dessert: "Disponible de 12h à 22h",
};
...
export function RoomServicePage() {
  ...
  useEffect(() => {
    if (!isLogged) {
      navigate("/connexion?redirect=/room-service");
      return;
    }
  ...
  const handleOrder = async () => {
    if (!roomNum.trim()) {
      setError("Veuillez saisir votre numero de chambre");
      return;
    }
        ...
        <div className="flex flex-col lg:flex-row max-w-none">
          ...
          <div className="w-full lg:w-[340px] flex-shrink-0 bg-white border-l border-border lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto flex flex-col">
            ...
            <div className="p-5 border-t border-border space-y-4">
              <div>
                <label className="block text-2xs tracking-luxury uppercase text-charcoal/40 mb-2">Numéro de chambre *</label>
                <input
                  type="text"
                  value={roomNum}
                  onChange={(e) => setRoomNum(e.target.value)}
                  placeholder="204"
                  className="w-full border border-border px-3 py-2.5 text-sm font-light text-charcoal outline-none focus:border-charcoal placeholder:text-charcoal/25 transition-colors"
                />
              </div>
              <div>
